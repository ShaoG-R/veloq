...
fn test_file_integrity() {
    crate::runtime::LocalExecutor::new().block_on(async {
        ...
        // 1. Create and Write
        {
            ...
            let mut write_buf = current_buffer_pool().upgrade().unwrap().alloc().unwrap();
        ...
        // 2. Open and Read
        {
            ...
            let mut read_buf = current_buffer_pool().upgrade().unwrap().alloc().unwrap();
